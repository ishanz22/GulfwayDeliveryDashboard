...
const PermissionManagement = () => {
  ...
  const filterDataByStatus = (status) => {
    ...
    const filteredItems = userPermissionData.filter((item) => item.status === status);
  ...
  const exportToExcel = () => {
    const dataToExport = userPermissionData.map((item) => ({
      ID: item.id,
      Name: item.name,
  ...
  const exportToPDF = () => {
    ...
    const tableData = userPermissionData.map((item) => ({
      ID: item.id,
      Name: item.name,
      Location: item.location,
      Earnings: `$${item.earnings}`,
          ...
          <Col xs="auto" className="d-flex align-items-end justify-content-end mb-2 mb-sm-0 order-sm-3">
            ...
            <Col xs="12" sm="auto" className="d-flex align-items-end justify-content-end mb-2 mb-sm-0 order-sm-3">
              <Button onClick={createUser} variant="outline-primary" className="btn-icon btn-icon-start w-100 w-md-auto">
                <CsLineIcons icon="plus" /> <span>Add Permission</span>
              </Button>
              <Button variant="outline-primary" className="btn-icon btn-icon-only ms-1 d-inline-block d-lg-none">
                <CsLineIcons icon="sort" />
              </Button>
